...
class PlacementTools:
    ...
    def list_my_applications(self, student_id: str) -> dict:
        ...
        Read-only: this tool does not create, modify, or book any placement records.
        Use it when the student asks to see their applications, application status,
        companies they applied to, or their booked interview details.

        student_id is the student's roll number used to find only that student's applications.
        """
        ...
        applications = self.repo.list_applications(s.id)
    ...
    def get_student(self, student_id: str) -> dict:
         """Get ONE student's placement profile using their roll number.

         Use when the user asks about a student's profile, CGPA, branch, backlogs,
         or graduation year.

         Read-only: this tool does not create or modify any placement records.
         Args:
                student_id: Roll number, e.g. "22CS045".

         Returns:
                Student details or unknown_student if the roll number does not exist.
    """
         s = self.repo.get_student(student_id)
    ...
    # "branch" rule that branch fails; same for grad_year. Ignore rules on other fields```python
    def list_open_drives(self, branch: str | None = None, grad_year: int | None = None) -> dict:
        """List currently open placement drives.

        Read-only: this tool does not create or modify applications.
        Use it when the user wants to explore available placement drives.
        Optionally filter the drives by branch or graduation year.
        Do not use it to check one student's eligibility for one drive.

        Args:
            branch: Optional branch filter, e.g. "CSE".
            grad_year: Optional graduation year filter, e.g. 2027.

        Returns:
            {"drives": [{"drive_id", "company", "role", "ctc_lpa", "deadline"}]}.
            Deadline is returned as YYYY-MM-DD.
        """    
        ...
        for drive in drives:
            ...
            for rule in self.repo.rules_for_drive(drive.id):
                if rule.field == "branch" and branch is not None:
                    if not _passes(rule, branch):
                        include = False

                if rule.field == "grad_year" and grad_year is not None:
                    if not _passes(rule, grad_year):
                        include = False
    ...
    def book_interview_slot(self, student_id: str, slot_id: int) -> dict:
        ...
        Side effect: reserves an interview slot for the student. Call it only when
        the user clearly chooses or asks to book a specific interview slot.
        Do not use it merely to view available slots.

        The checks happen in this exact order:
        unknown_student, unknown_slot, no_application, then slot_taken.

        Args:
            student_id: Roll number, e.g. "22CS045".
            slot_id: Integer interview slot id.
        ...
        Returns:
            ...
            return slot_taken and include the drive's remaining available slots.
        """
        s = self.repo.get_student(student_id)
        ...
        has_application = self.repo.has_application(s.id, slot.drive_id)

        if not has_application:
            return {
                "error": "no_application",
                "hint": "The student has not applied to this drive. Apply to the drive before booking an interview slot."
            }
        ...
        if not claimed:
            ...
            return {
                ...
                "hint": "That slot was already taken. Offer the remaining available slots."
            }
    ...
    def notify_student(self, student_id: str, message: str) -> dict:
        """Send one notification to a student.

        Side effect: queues exactly one notification through the configured
        notifier. Requires a valid student_id and a non-empty message of at most
        160 characters. Returns the queued notification ID and status.
        """
