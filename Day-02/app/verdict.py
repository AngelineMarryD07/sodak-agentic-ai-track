...
class EligibilityVerdict(BaseModel):
    ...
    def check_failed_rules(self):
        if self.eligible and self.failed_rules:
            raise ValueError("eligible verdict contradicts failed_rules")
        if not self.eligible and not self.failed_rules:
            raise ValueError("ineligible verdict contradicts failed_rules")
        return self
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt == max_retries:
                raise VerdictInvalid(attempt + 1, exc.errors())

            messages.append(raw)
            messages.append(
                f"Your previous response failed validation: {exc}. "
                "Please return a corrected JSON verdict."
            )

    raise VerdictInvalid(max_retries + 1, [])
