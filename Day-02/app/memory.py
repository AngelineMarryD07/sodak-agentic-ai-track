...
class ConversationStore:
    ...
    def append_message(self, thread_id: str, role: str, text: str) -> int:
        """Insert a message with the next sequence number and return that seq."""
        cursor = self.conn.execute(
            """
            INSERT INTO message (thread_id, seq, role, text)
            VALUES (
                ?,
                COALESCE((SELECT MAX(seq) FROM message WHERE thread_id = ?), 0) + 1,
                ?,
                ?
            )
            """,
        ...
        return self.conn.execute(
           "SELECT seq FROM message WHERE id = ?",
           (cursor.lastrowid,)
        ).fetchone()["seq"]

    def load_history(self, thread_id: str) -> list[dict]:
        """Return the thread's messages in sequence order."""
    ...
    def start_run(self, thread_id: str, model: str) -> str:
        """Create a new running agent run and return its id."""
        run_id = str(uuid.uuid4())

        self.conn.execute(
            """
            INSERT INTO run (id, thread_id, status, model)
            VALUES (?, ?, 'running', ?)
            """,
    ...
    def record_model_step(self, run_id: str, seq: int, tokens_in: int, tokens_out: int) -> int:
        """Record a model step and add its tokens to the run in one transaction."""
    ...
    def record_tool_call(self, run_id: str, seq: int, name: str, args: dict, result: dict,
        ...
        """Record a tool step and its tool call in one transaction."""
        with self.transaction() as conn:
            cursor = conn.execute(
                """
    ...
    def finish_run(self, run_id: str, status: str, error_code: str | None = None) -> None:
        """Finish a run with its final status and optional error code."""
        self.conn.execute(
            """
            UPDATE run
            SET status = ?,
    ...
    def page_messages(self, thread_id: str, after_seq: int = 0, limit: int = 20) -> tuple[list[dict], int | None]:
        """Return one page of messages after after_seq."""
        if limit <= 0:
        ...
        if len(items) < limit:
            ...
            more = self.conn.execute(
                ...
                LIMIT 1
                """,
                (thread_id, items[-1]["seq"]),
            ).fetchone()

            next_after = items[-1]["seq"] if more else None
