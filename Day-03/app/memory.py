...
class RunStore:
    ...
    def claim_next(self, worker_id: str, lease_seconds: float) -> Claimed | None:
        ...
        with self.transaction() as c:
            ...
            c.execute(f"UPDATE run SET status = 'running', lease_owner = ?, lease_until = ?, attempts = ?, "
    ...
    def heartbeat(self, run_id: str, worker_id: str, lease_seconds: float) -> bool:
        ...
        cur = self.conn.execute("UPDATE run SET lease_until = ? "
    ...
    def reap_expired(self) -> list[str]:
        ...
        with self.transaction() as c:
            rows = c.execute("SELECT id, attempts, max_attempts FROM run "
                             "WHERE status = 'running' AND lease_until < ? ORDER BY created_at", (now,)).fetchall()
            for row in rows:
                touched.append(row["id"])
                if row["attempts"] < row["max_attempts"]:
                    c.execute("UPDATE run SET status = 'queued', available_at = ?, error_code = 'lease_expired', "
                              "lease_owner = NULL, lease_until = NULL WHERE id = ?", (now, row["id"]))
                else:
        ...
        with self.transaction() as c:
            ...
            if status == "queued":
                ...
                c.execute("UPDATE run SET status = 'queued', error_code = ?, lease_owner = NULL, lease_until = NULL, "
                          "available_at = ?, finished_at = NULL WHERE id = ?",
